...
colorBtn.addEventListener("click", () => {
  ...
  fetch(
    `https://www.thecolorapi.com/scheme?hex=${colorHash}&mode=${colorSchemes}&count=5`
  )
    .then((res) => res.json())
    .then((data) => {
      displayColors(data);
...
function displayColors(colorObject) {
  ...
  colorsArray.map((colorArray) => {
    html += `
        <div style="background-color: ${colorArray.hex.value}; min-width: 20%;  align-items: flex-end; display: flex; justify-content: center;">${colorArray.hex.value}</div>
        `;
