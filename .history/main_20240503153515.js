...
function displayColors(colorObject) {
  ...
  colorsArray.map((colorArray) => {
    html += `
        <div class="colorColumn" style="background-color: ${colorArray.hex.value}; min-width: 20%;  align-items: flex-end; display: flex; justify-content: center; cursor: pointer; padding: 12px;"><span style="background-color:white; padding: 12px;">${colorArray.hex.value}</span></div>
        `;
    ...
    colorColumns.forEach((colorColumn) => {
      attachCopyText(colorColumn);
    });
  });
}

function attachCopyText(colorColumn) {
