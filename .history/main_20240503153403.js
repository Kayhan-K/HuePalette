...
function displayColors(colorObject) {
  ...
  colorsArray.map((colorArray) => {
    html += `
        <div class="colorColumn" style="background-color: ${colorArray.hex.value}; min-width: 20%;  align-items: flex-end; display: flex; justify-content: center; cursor: pointer; padding: 12px;">${colorArray.hex.value}</div>
        `;
